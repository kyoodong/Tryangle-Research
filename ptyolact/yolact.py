...
from ptyolact.data.config import cfg, mask_type
from ptyolact.layers import Detect
from ptyolact.layers.interpolate import InterpolateModule
from ptyolact.backbone import construct_backbone

import torch.backends.cudnn as cudnn
from ptyolact.utils import timer
from ptyolact.utils.functions import MovingAverage, make_net
...
if __name__ == '__main__':
    from ptyolact.utils.functions import init_console
    init_console()

    # Use the first argument to set the config if you want
    import sys
    if len(sys.argv) > 1:
        from ptyolact.data.config import set_cfg
        set_cfg(sys.argv[1])
